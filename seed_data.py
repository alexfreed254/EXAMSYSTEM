"""
Seed script for TTTI ERMS - populates initial data including
Electrical Engineering Level 6 curriculum per TVET CDACC guidelines.
"""
...
def seed():
    ...
    with app.app_context():
        ...
        if not User.query.filter_by(username='admin').first():
            admin = User(username='admin', email='admin@ttti.ac.ke',
        ...
        # ─── Academic Year ───────────────────────────────────────────────────
        if not AcademicYear.query.filter_by(name='2024/2025').first():
        ...
        for name, code, desc in departments_data:
            if not Department.query.filter_by(code=code).first():
                ...
                dept_map[code] = Department.query.filter_by(code=code).first().id

        # ─── Program: Electrical Engineering Level 6 ─────────────────────────
        if not Program.query.filter_by(code='EE-L6-C3').first():
            prog = Program(
                ...
                duration_years=3,
                description='Competency-Based Modular Curriculum for Electrical Engineering KNQF Level 6 Cycle 3',
                entry_requirements='KCSE Grade C- (minus) or Certificate in Electrical Engineering KNQF Level 5 or equivalent qualifications as determined by TVETA',
            )
            db.session.add(prog)
            db.session.flush()
            prog_id = prog.id
            ...
            prog_id = Program.query.filter_by(code='EE-L6-C3').first().id

        # ─── Courses (Units) ─────────────────────────────────────────────────
        # Format: (code, tvet_code, name, module, category, theory%, hours, credits)
        units = [
            ...
            ('0541 441 05B-II', 'ENG/CU/EI/CC/07/5/MB', 'Engineering Technician Mathematics II', 5, 'CC', 30, 100, 10.0),
        ...
        for code, tvet_code, name, module, category, theory_w, hours, credits in units:
            if not Course.query.filter_by(code=code, program_id=prog_id).first():
                course = Course(
                    name=name, code=code, tvet_unit_code=tvet_code,
                    program_id=prog_id, module_number=module,
                    unit_category=category, theory_weight=theory_w,
                    practical_weight=100 - theory_w,
                    duration_hours=hours, credit_factor=credits,
                )
                db.session.add(course)

        print(f"✓ {len(units)} units/courses seeded for Electrical Engineering Level 6")

        # ─── Sample Trainer ───────────────────────────────────────────────────
        if not User.query.filter_by(username='trainer1').first():
            t_user = User(username='trainer1', email='trainer1@ttti.ac.ke',
                          full_name='John Kamau Mwangi', role='trainer', phone='0712345678')
            t_user.set_password('Trainer@2025')
            db.session.add(t_user)
            db.session.flush()
            trainer = Trainer(user_id=t_user.id, department_id=dept_map['EE'],
        ...
        # ─── Sample Trainee ───────────────────────────────────────────────────
        if not User.query.filter_by(username='trainee1').first():
            s_user = User(username='trainee1', email='trainee1@ttti.ac.ke',
            ...
            prog = Program.query.filter_by(code='EE-L6-C3').first()
            if year and prog:
                enrollment = Enrollment(trainee_id=trainee.id, program_id=prog.id,
